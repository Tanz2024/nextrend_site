...
import data from "../content/monitors.json";
import { buildKArrayMonitorUrl } from "@/lib/assets";

const mapProduct = (raw: any): KArrayProduct => {
  const mapLifestyle = (items: any[]) =>
    (items || []).map((item) => ({
      src: buildKArrayMonitorUrl(item.src),
      alt: item.alt || "",
  ...
  const mapResources = (items: any[]) =>
    (items || [])
      .map((r) => {
        ...
        if (!href) return null;
        return { label, href: buildKArrayMonitorUrl(href) };
      })
      .filter(Boolean);
  ...
  return {
    ...
    image: buildKArrayMonitorUrl(raw.image),
...
export const monitorsProducts: KArrayProduct[] = (data as any[]).map(mapProduct);
