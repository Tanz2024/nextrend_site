...
const mapProduct = (raw: any): KArrayProduct => {
  ...
  const mapResources = (items: any[]) =>
    (items || [])
      .map((r) => {
        ...
        return {
          label,
          href: buildKArrayUrl(`K_array_images/light_images/${href}`),
        };
      })
      .filter(Boolean);
  ...
  return {
    ...
    image: buildKArrayUrl(`K_array_images/light_images/${raw.image}`),
...
export const lightProducts: KArrayProduct[] = (data as any[]).map(mapProduct);
