...
import data from "../content/systems.json";
import { buildKArraySystemUrl } from "@/lib/assets";

const mapProduct = (raw: any): KArrayProduct => {
  const mapLifestyle = (items: any[]) =>
    (items || []).map((item) => ({
      src: buildKArraySystemUrl(item.src),
      alt: item.alt || "",
  ...
  const mapResources = (items: any[]) =>
    (items || [])
      .map((r) => {
        ...
        if (!href) return null;
        return { label, href: buildKArraySystemUrl(href) };
      })
      .filter(Boolean);
  ...
  return {
    ...
    image: buildKArraySystemUrl(raw.image),
...
export const systemsProducts: KArrayProduct[] = (data as any[]).map(mapProduct);
