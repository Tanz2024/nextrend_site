// app/products/amina/[slug]/page.tsx
// @ts-nocheck
"use client";
...
import { aminaProducts as productData } from "../data";
...
export default function ProductPage({ params }: Props) {
  ...
  const metaLine = metaParts.filter((part, index, arr) => {
  ...
  function ContactCTA({ className = "" }: { className?: string }) {
    return (
      <Link
        href="/contact"
        className={[
          "group inline-flex items-center gap-2 py-3 text-sm sm:text-base md:text-[1.15rem] lg:text-[1.5rem] xl:text-[1.65rem] font-[500] md:font-light tracking-[0.01em] md:tracking-[-0.02em] leading-[1.45] text-[#1A1A1A]/80 transition-colors duration-300 hover:text-[#1A1A1A] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--champagne)]/40",
          className,
        ]
          .filter(Boolean)
          .join(" ")}
      >
        ...
        <svg
          ...
          width="16"
          height="16"
          className="text-current h-4 w-4 transition-transform duration-300 group-hover:translate-x-1"
        >
          <path
            d="M7 5l9 7-9 7"
            fill="none"
      ...
      >
        <section className="relative z-10 mx-auto w-full max-w-[1280px] px-4 pb-8 pt-4 sm:px-6 sm:pb-16 sm:pt-8 md:px-12 md:pb-20 md:pt-12 lg:px-20">
          ...
          >
            ...
            <Link href="/products/amina" className="hover:text-[#C4A777]">
              {product.brand ?? "Amina"}
            </Link>
            <span className="mx-1 sm:mx-2 opacity-30">/</span>
 ...
 */}
          ...
          <ProductNavButtons
            prev={prev ?? undefined}
            next={next ?? undefined}
            resolveHref={(link) => "/products/amina/" + link.slug}
          />
        </section>
