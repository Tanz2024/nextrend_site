...
import { trinnovProducts } from "./data";

const loadMotionFeatures = () =>
  import("framer-motion").then((mod) => mod.domAnimation);

const trinnovHeroFont = Bodoni_Moda({
  subsets: ["latin"],
  weight: ["400", "500", "600", "700"],
});

const trinnovProductFont = Playfair_Display({
  subsets: ["latin"],
  weight: ["400", "500", "600", "700"],
});
...
const getDisplayParts = (name: string, series: string) => {
  // Use last part after " - " as model name if present
  const title = (name.split(" - ").pop() || name).trim();
  return { title, subtitle: series?.toUpperCase() ?? "" };
};

const products = trinnovProducts.map((product) => ({
  slug: product.slug,
  name: product.name,
...
const filtersConfig = [
  {
    key: "series",
    title: "Series",
    options: unique(products.map((p) => p.series)),
  },
  {
    key: "finish",
    title: "Finish",
...
const IconFinishHeader = ({ className, ...p }: any) => (
  <svg viewBox="0 0 20 20" width="18" height="18" className={className} {...p}>
    {/* “sparkle / finish” */}
    <path
      d="M10 3.2l1.1 2.7 2.8 1.1-2.8 1.1L10 10.8 8.9 8.1 6.1 7l2.8-1.1L10 3.2Z"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.3"
      strokeLinecap="round"
...
const IconCollabHeader = ({ className, ...p }: any) => (
  <svg viewBox="0 0 20 20" width="18" height="18" className={className} {...p}>
    {/* “interlocking rings / collaboration” */}
    <circle
      cx="8"
      cy="10"
      r="3.3"
...
/* ---------- page ---------- */
export default function TrinnovCollection() {
        ...
        {expandedGroups[f.title] && (
          ...
          >
            <div className="px-4 pb-3 pt-1 space-y-2">
              {f.options.map((opt) => {
                const checked = filters[f.key as FilterKey].includes(opt);
                return (
                  <div key={opt}>
                    {isDesktop ? (
      ...
      <main className="relative min-h-screen bg-transparent text-[#1A1A1A]">
        <section
          className="relative w-full max-w-[1440px] mx-auto px-4 pt-12 pb-10
                    ...
                    lg:px-10 lg:pt-24"
        >
          {/* Hero - same layout as KGEAR page */}
          <m.div
          ...
          >
            <h1
              className={`${trinnovHeroFont.className} text-[clamp(2.6rem,6vw,4.3rem)] font-light leading-[1.02] tracking-[-0.02em] text-[#1A1A1A]`}
            >
              TRINNOV Products
            </h1>

            <p className="text-[0.95rem] sm:text-[1.02rem] lg:text-[1.06rem] leading-[1.8] text-[rgba(26,26,26,0.7)] max-w-3xl">
              Altitude processors, Amplitude power amplifiers, and Nova studio tools, precision-engineered for cinema, mastering rooms, and reference listening spaces. Explore by series, finish, or special collaborations.
            </p>
          </m.div>
          ...
          >
            {/* Toolbar - spans full width, same as KGEAR */}
...
return (
  ...
  >
                      ...
                      <Link
                        href={`/products/trinnov/${item.slug}`}
                        className="group relative block w-full bg-white touch-manipulation active:scale-[0.98] transition-transform overflow-hidden aspect-[4/3]"
                      >
                        <Image
                          src={item.image}
                          alt={item.name}
                      ...
                      <div className="flex flex-1 flex-col px-5 sm:px-6 lg:px-9 pt-3 sm:pt-4 lg:pt-5 pb-16 sm:pb-18 lg:pb-20 text-left">
                        <div className="max-w-[30rem]">
                          <h2
                            className={`${trinnovProductFont.className} text-[1.8rem] sm:text-[2rem] lg:text-[2.35rem] leading-[1.05] font-light text-[#1A1A1A]`}
                          >
                            <Link
                              href={`/products/trinnov/${item.slug}`}
              ...
              {/* Pagination (same pattern as KGEAR) */}
              ...
              {!productsList.length && (
                <div className="rounded-[16px] sm:rounded-[20px] lg:rounded-[24px] bg-white/70 px-4 sm:px-6 py-8 sm:py-10 text-center text-[0.85rem] sm:text-sm text-[rgba(26,26,26,0.6)] shadow">
                  No Trinnov editions match the current filters. Adjust your
                  selection or{" "}
                  <Link
                    href="/contact"
        ...
        {/* Mobile Filter Drawer - same pattern as KGEAR */}
                    ...
                    <div className="mb-6">
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder="Search Trinnov products..."
