...
const mapProduct = (raw: any): FrogisProduct => {
  ...
  return {
    ...
    image: buildFrogisUrl(`Point source speakers/${raw.image}`),
    lifestyle: mapLifestyle(raw.lifestyle || []),
  };
};

export const pointSourceProducts: FrogisProduct[] = (data as any[]).map(mapProduct);
