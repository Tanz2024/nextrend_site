...
import data from "../content/slimline_arrays.json";
import { buildFrogisUrl } from "@/lib/assets";

const mapProduct = (raw: any): FrogisProduct => {
  const mapLifestyle = (items: any[]) =>
    (items || []).map((item) => ({
      src: buildFrogisUrl(`Slimline array speaker systems/${item.src}`),
      alt: item.alt || "",
  ...
  return {
    ...
    image: buildFrogisUrl(`Slimline array speaker systems/${raw.image}`),
    lifestyle: mapLifestyle(raw.lifestyle || []),
  };
};

export const slimlineArraysProducts: FrogisProduct[] = (data as any[]).map(mapProduct);
