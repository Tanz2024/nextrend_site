...
const mapProduct = (raw: any): FrogisProduct => {
  const mapLifestyle = (items: any[]) =>
    (items || []).map((item) => ({
      src: buildFrogisUrl(`Subwoofers/${item.src}`),
      alt: item.alt || "",
  ...
  return {
    ...
    image: buildFrogisUrl(`Subwoofers/${raw.image}`),
    lifestyle: mapLifestyle(raw.lifestyle || []),
  };
};

export const subwoofersProducts: FrogisProduct[] = (data as any[]).map(mapProduct);
