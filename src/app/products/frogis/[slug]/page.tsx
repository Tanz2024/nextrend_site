// app/products/frogis/[slug]/page.tsx
// @ts-nocheck
"use client";
...
import { allFrogisProducts as productData } from "../data/index";
      ...
      >
        <div className="grid gap-10 md:grid-cols-2 md:gap-x-9 lg:gap-x-10 2xl:gap-x-12">
          {groups.map((group, index) => {
            ...
            return (
              <section key={`${title}-${index}`} className="min-w-0">
                <div>
      ...
      >
        <section className="relative z-10 mx-auto w-full max-w-[1280px] px-4 pb-8 pt-4 sm:px-6 sm:pb-16 sm:pt-8 md:px-12 md:pb-20 md:pt-12 lg:px-20">
          ...
          >
            ...
            <Link href="/products/frogis" className="hover:text-[#C4A777]">
              {product.brand ?? "Frogis"}
            </Link>
            <span className="mx-1 sm:mx-2 opacity-30">/</span>
 ...
 */}
          ...
          <ProductNavButtons
            prev={prev ?? undefined}
            next={next ?? undefined}
            resolveHref={(link) => "/products/frogis/" + link.slug}
          />
        </section>
