...
import { allFrogisProducts as frogisProducts } from "./data/index";

const loadMotionFeatures = () =>
  import("framer-motion").then((mod) => mod.domAnimation);

/* ---------- fonts ---------- */
const frogisHeroFont = Bodoni_Moda({
  subsets: ["latin"],
  weight: ["400", "500", "600", "700"],
});

const frogisProductFont = Playfair_Display({
  subsets: ["latin"],
  weight: ["400", "500", "600", "700"],
});

/* ---------- helpers ---------- */
const getDisplayParts = (name: string, category: string) => {
  const title = (name.split(" - ").pop() || name).trim();
  return { title, subtitle: category?.toUpperCase() ?? "" };
};

const getModelNumber = (name: string) =>
  name.replace(/^Frog[-\s]?is\s+/i, "").trim();

const products = frogisProducts.map((product) => ({
  slug: product.slug,
  name: product.name,
  ...
  modelNumber: getModelNumber(product.name),
...
type FilterKey = (typeof filterKeys)[number]["key"];
...
const sortOptions = [
  {
    key: "recommended",
    label: "Recommended",
    description: "Curated Frog-Is order",
  },
  {
    key: "name-asc",
    label: "Name A-Z",
  ...
  },
  {
    key: "series",
    label: "Series",
    description: "Grouped by product category",
  },
] as const;
...
const filterHeaderIcons: any = {
  category: IconSeriesHeader,
  model: IconSeriesHeader,
...
/* ---------- pagination helper (same as Brionvega) ---------- */
...
/* ---------- page ---------- */
export default function FrogisCollection() {
  ...
  /* read initial page from ?page= query on first mount */
  useEffect(() => {
    ...
    if (!Number.isNaN(numeric) && numeric > 0) {
      // URL uses 1-based (1,2,3...), state uses 0-based (0,1,2...)
      setCurrentPage(numeric - 1);
    }
  }, []);

  /* keep ?page= in sync whenever currentPage changes */
  useEffect(() => {
    if (typeof window === "undefined") return;
    ...
    if (currentPage === 0) {
      // page 1 -> clean URL: /products/frogis
      params.delete("page");
    } else {
      // page 2,3,... -> /products/frogis?page=2,3,...
  ...
  const productsList = useMemo(() => {
    const filtered = products.filter((item) => {
      ...
      const matchesMain = filterKeys.every(({ key }) => {
        const active = filters[key as FilterKey];
        let value = (item as any)[key] as string | undefined;
        if (key === "model") {
  ...
  const modelOptions = useMemo(() => {
    ...
    const source = selectedCategories.length
      ...
      : products;
    return unique(source.map((product) => product.modelNumber));
  }, [filters.category]);
    ...
    () => [
      {
        ...
        title: "Series",
        options: frogisSeriesOrder.filter((label) =>
          products.some((p) => p.category === label)
        ),
      },
      {
        key: "model" as const,
  ...
  /* grid classes – same as Brionvega (1–2 cols) */
      ...
      <main className="relative min-h-screen bg-transparent text-[#1A1A1A]">
        <section
          className="relative w-full max-w-[1440px] mx-auto px-4 pt-12 pb-10
                    ...
                    lg:px-10 lg:pt-20"
        >
          {/* Hero (same layout as Brionvega, no logo) */}
          <m.div
          ...
          >
            <h1
              className={`${frogisHeroFont.className} text-[clamp(2.4rem,6vw,4.3rem)] font-light leading-[1.02] tracking-[-0.02em] text-[#1A1A1A]`}
            >
              FROG-IS Products
            </h1>

            <p className="text-[0.95rem] sm:text-[1.02rem] lg:text-[1.06rem] leading-[1.8] text-[rgba(26,26,26,0.7)] max-w-3xl">
              Stainless line arrays, Dante ceilings, weatherized subwoofers and
              reference monitors curated for hospitality, touring and
              architectural deployments. Filter by series or finish to narrow
              your short-list.
            </p>
          </m.div>
            ...
            <div className="space-y-6 sm:space-y-8 pt-4">
              ...
              >
                {displayed.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Link
                        href={`/products/frogis/${item.slug}`}
                      ...
                      <div className="flex flex-1 flex-col px-5 sm:px-6 lg:px-9 pt-3 sm:pt-4 lg:pt-5 pb-16 sm:pb-18 lg:pb-20 text-left">
                        <div className="max-w-[30rem]">
                          <h2
                            className={`${frogisProductFont.className} text-[1.8rem] sm:text-[2rem] lg:text-[2.35rem] leading-[1.05] font-light text-[#1A1A1A]`}
                          >
                            <Link
                              href={`/products/frogis/${item.slug}`}
                          ...
                          <p className="mt-3 sm:mt-4 lg:mt-5 text-[0.9rem] sm:text-[0.95rem] lg:text-[1rem] leading-[1.9] text-[rgba(26,26,26,0.68)]">
                            {item.definition || item.headline || item.description}
                          </p>
                        </div>
                      </div>
              ...
              {!productsList.length && (
                <div className="rounded-[16px] sm:rounded-[20px] lg:rounded-[24px] bg-white/70 px-4 sm:px-6 py-8 sm:py-10 text-center text-[0.85rem] sm:text-sm text-[rgba(26,26,26,0.6)] shadow">
                  No Frog-Is systems match the current filters. Adjust your
                  selection or{" "}
                  <Link
                    href="/contact"
        ...
        {/* Mobile Filter Drawer (same shell as Brionvega) */}
                    ...
                    <div className="mb-6">
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder="Search Frog-Is products..."
