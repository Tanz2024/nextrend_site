// app/products/bearbricks/[slug]/page.tsx
// @ts-nocheck
"use client";
...
import { bearbrickProducts as productData } from "../data";
...
const heroVideoSrc =
  (activeFinish as any)?.video ||
  (product as any).video ||
  (product as any).heroVideo?.src;
const heroVideoPoster = (product as any).heroVideo?.poster;
...
>
  ...
  return (
    <LazyMotion features={loadMotionFeatures} strict>
      ...
      >
    <section className="relative z-10 mx-auto w-full max-w-[1440px] 2xl:max-w-[1536px] px-4 pb-8 pt-4 sm:px-6 sm:pb-16 sm:pt-8 md:px-12 md:pb-20 md:pt-12 lg:px-20">
          ...
          >
            ...
            <Link href="/products/bearbricks" className="hover:text-[#C4A777]">
              {product.brand ?? "Bearbricks"}
            </Link>
            <span className="mx-1 sm:mx-2 opacity-30">/</span>
          ...
          {/* hero: media + meta */}
      <div className="grid items-start gap-6 sm:gap-8 md:gap-10 lg:gap-12 lg:grid-cols-[minmax(0,54%)_minmax(0,46%)]">
...
{galleryShots.length > 0 && (
  <div className="mt-6 hidden md:block">
    {(() => {
      ...
      const GAP = 16; // gap-4
      const cols = Math.min(4, galleryShots.length || 1);
     ...
     <div className="space-y-4 sm:space-y-5 lg:space-y-7">
              ...
              >
          {product.series} · {finishLabel} · {(product as any).collection || product.collaboration}
              ...
              <m.h1
                {...fadeUp}
             className="
  font-[400] md:font-light
  text-[#0f0f0f] md:text-[#111]
  text-balance
tracking-[-0.028em] sm:tracking-[-0.03em] lg:tracking-[-0.032em] 2xl:tracking-[-0.034em]
leading-[1.04] sm:leading-[1.035] lg:leading-[1.025] 2xl:leading-[1.015]


  /* mobile */
  text-[clamp(2.2rem,9vw,3.1rem)]

  /* laptop (md–xl) ✅ keep controlled */
  md:text-[clamp(2.5rem,3.6vw,3.35rem)]
  lg:text-[clamp(2.7rem,3.0vw,3.55rem)]
  xl:text-[clamp(2.9rem,2.6vw,3.75rem)]

  /* big monitor */
  2xl:text-[clamp(3.2rem,3.2vw,4.4rem)]
"


              >
    {(() => {
  const top = (product.name || "").trim();                 // "BE@RBRICK Audio 400%"
  const bottom = ((product as any).collection || "").trim(); // "Clear Collection"
  ...
  className={[
    "mt-1 block",
    // sizing ONLY for collection line
    "text-[clamp(1.9rem,7.6vw,2.85rem)]",
    "md:text-[clamp(2.05rem,3.2vw,3.05rem)]",
    "lg:text-[clamp(2.15rem,2.6vw,3.2rem)]",
    "2xl:text-[clamp(2.35rem,2.3vw,3.45rem)]",
    // tighter + more engineered
    "tracking-[-0.018em] sm:tracking-[-0.02em] lg:tracking-[-0.022em]",
    "leading-[1.02]",
    // gradient text
    "text-transparent bg-clip-text [-webkit-background-clip:text]",
    // minimal glow (not neon)
    "[text-shadow:0_0_14px_rgba(196,167,119,0.18)] sm:[text-shadow:0_0_18px_rgba(196,167,119,0.16)]",
  ].join(" ")}
  style={{
    backgroundImage:
      "linear-gradient(180deg, rgba(196,167,119,0.98) 0%, rgba(196,167,119,0.78) 55%, rgba(196,167,119,0.96) 100%)",
  }}
>
  {bottom}
</span>
  ...
  text-[clamp(0.98rem,3.6vw,1.08rem)]
  md:text-[clamp(1.02rem,1.25vw,1.14rem)]
  2xl:text-[clamp(1.08rem,1.05vw,1.18rem)]
"
...
<p className="max-w-[68ch] text-[0.98rem] leading-[1.85] text-black/72 whitespace-pre-line">
  {product.story}
          ...
          {/* lifestyle / scene */}
     {(heroVideoSrc || (activeLifestyle?.length > 0)) && (
 ...
 */}
          ...
          <ProductNavButtons
            prev={prev ? { ...prev, subtitle: prev.collection } : undefined}
            next={next ? { ...next, subtitle: next.collection } : undefined}
            resolveHref={(link) => "/products/bearbricks/" + link.slug}
          />
        </section>
