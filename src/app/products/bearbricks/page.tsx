// app/products/bearbricks/page.tsx
// @ts-nocheck
"use client";
...
import { buildBearbricksProductUrl, buildBearbricksLifestyleUrl } from "@/lib/assets";
import { bearbrickProducts } from "./data";

const loadMotionFeatures = () =>
  import("framer-motion").then((mod) => mod.domAnimation);

/* ---------- fonts ---------- */
const bearbrickHeroFont = Bodoni_Moda({
  subsets: ["latin"],
  weight: ["400", "500", "600", "700"],
});

const bearbrickProductFont = Playfair_Display({
  subsets: ["latin"],
  weight: ["400", "500", "600", "700"],
});
...
const products = bearbrickProducts.map((product) => ({
  slug: product.slug,
  name: product.name,
  description: product.description,
  series: product.series,
  displaySeries: product.displaySeries ?? product.series,
  collection: (product as any).collection ?? "", // ✅ add this
  seriesVariants: unique(
    (product.finishes || []).map((finish: any) => finish?.series).filter(Boolean)
  ),
...
const filtersConfig = [
  {
    ...
    options: unique([
      ...
      ...products.flatMap((p) => p.seriesVariants || []),
    ]),
  },
  {
    key: "finish",
    title: "Finish",
    options: unique(products.map((p) => p.finish)),
  },
  {
    key: "collaboration",
    title: "Collaboration",
    options: unique(products.map((p) => p.collaboration)),
  },
] as const;
...
const sortOptions = [
  {
    key: "recommended",
    label: "Recommended",
    description: "Curated Bearbrick order",
  },
  {
    key: "name-asc",
    label: "Name A-Z",
...
/* Header icons per group (like Frog-is) */
...
const IconFinishHeader = ({ className, ...p }: any) => (
  <svg viewBox="0 0 20 20" width="18" height="18" className={className} {...p}>
    <rect
      x="3"
      y="4"
...
/* ---------- pagination helper (DOTS, like Frog-is) ---------- */
...
/* ---------- page ---------- */
export default function BearbrickCollection() {
  ...
  const productsList = useMemo(() => {
    const filtered = products.filter((item) => {
      ...
      const matchesAll = filtersConfig.every(({ key }) => {
      const active = filters[key as FilterKey];
      const value = (item as any)[key] as string | undefined;
      if (key === "series") {
        if (!active.length) return true;
        const variants = (item as any).seriesVariants || [];
        return active.some(
          (entry) => entry === value || variants.includes(entry)
        );
      }
      return !active.length || (value && active.includes(value));
    });
...
const gridWrapperClasses =
  "grid grid-cols-1 gap-6 sm:gap-8 md:gap-10 sm:grid-cols-2 lg:grid-cols-2";


  return (
    <LazyMotion features={loadMotionFeatures} strict>
      <main className="relative min-h-screen bg-transparent text-[#1A1A1A]">
        {/* Mobile Filter Drawer (same shell as Frog-is) */}
                    ...
                    <div className="mb-6">
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder="Search BE@RBRICK editions..."
        ...
        <section
          className="relative w-full max-w-[1440px] mx-auto px-4 pt-12 pb-10
                    ...
                    lg:px-10 lg:pt-20"
        >
          {/* Hero – no big logo, text-based like Frog-is/Brionvega */}
          <m.div
          ...
          >
            <h1
              className={`${bearbrickHeroFont.className} text-[clamp(2.4rem,6vw,4.3rem)] font-light leading-[1.02] tracking-[-0.02em] text-[#1A1A1A]`}
            >
              BE@RBRICK Products
            </h1>

            <p className="text-[0.95rem] sm:text-[1.02rem] lg:text-[1.06rem] leading-[1.8] text-[rgba(26,26,26,0.7)] max-w-3xl">
              Sculptural loudspeakers, studio editions and marquee
              collaborations designed for listening salons, private lounges and
              gallery-grade hospitality. Filter by series, finish or
              collaboration to build your shortlist.
            </p>
          </m.div>
                ...
                {displayed.map((item) => {
                  ...
                  return (
                    <m.article
                      ...
                      variants={cardIn}
                      layout
                      className="group/card relative flex h-full flex-col overflow-hidden rounded-[20px] sm:rounded-[24px] lg:rounded-[28px] bg-white shadow-[0_16px_40px_rgba(26,26,26,0.08)] sm:shadow-[0_20px_60px_rgba(26,26,26,0.10)] lg:shadow-[0_26px_80px_rgba(26,26,26,0.12)] transition-shadow duration-500 touch-manipulation"
                    >
                  <Link
  href={`/products/bearbricks/${item.slug}`}
  className="relative block w-full bg-white touch-manipulation active:scale-[0.98] transition-transform h-[260px] sm:h-[300px] lg:h-[340px]"
>
                      ...
                      <div className="flex flex-1 flex-col px-5 sm:px-6 lg:px-9 pt-3 sm:pt-4 lg:pt-5 pb-16 sm:pb-18 lg:pb-20 text-left">
                        <div className="max-w-[30rem]">
                          <h2
                            className={`${bearbrickProductFont.className} text-[1.8rem] sm:text-[2rem] lg:text-[2.35rem] leading-[1.05] font-light text-[#1A1A1A]`}
                          >
                            <Link
                              href={`/products/bearbricks/${item.slug}`}
              ...
              {!productsList.length && (
                <div className="rounded-[16px] sm:rounded-[20px] lg:rounded-[24px] bg-white/70 px-4 sm:px-6 py-8 sm:py-10 text-center text-[0.85rem] sm:text-sm text-[rgba(26,26,26,0.6)] shadow">
                  No BE@RBRICK editions match the current filters. Adjust your
                  selection or{" "}
                  <Link
                    href="/contact"
