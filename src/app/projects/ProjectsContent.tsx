...
const TILE_CONFIG: TileConfig[] = [
  ...
  { label: 'Hospitality', sectionKey: 'Hospitality', valueKey: 'hospitality', icon: ForkKnife },
  { label: 'Residential', sectionKey: 'Residential', valueKey: 'residential', icon: HouseLine },
  { label: 'Commercial & Retail', sectionKey: 'Commercial & Retail', valueKey: 'commercialRetail', icon: ShoppingBag },
  { label: 'Entertainment & Venues', sectionKey: 'Entertainment & Venues', valueKey: 'entertainmentVenues', icon: Factory },
];
...
>
  ...
  <div className="relative z-10 flex items-center gap-3">
    ...
    <span className="relative text-[11px] font-semibold uppercase tracking-[0.2em] leading-[1.5] text-[#0d0d0c] transition-colors duration-300 group-hover:text-[#d5a853]">
      ...
      <i
        aria-hidden
        className="absolute left-0 -bottom-1 h-[1px] w-0 rounded-full bg-[#d5a853] transition-all duration-300 group-hover:w-full"
      />
    </span>
  </div>
  ...
  {/* Bottom: value only */}
  <div
    className="relative z-10 mt-6 tabular-nums tracking-[-0.02em] text-[2rem] leading-none text-[#d5a853] md:mt-8 md:text-[2.4rem]"
    ...
    >
      <span className="relative text-[0.72rem] sm:text-[0.75rem] font-medium uppercase tracking-[0.28em] inline-block">
        <span className="block">
          {tilesExpanded ? "Show fewer categories" : "Explore all categories"}
        </span>
